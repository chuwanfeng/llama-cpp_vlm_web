...
from config import (
    ...
    MODELS_DIR, GPU_DEFAULT_LAYERS, FAMILY_CONTEXT,


    DEFAULT_TEMPERATURE, DEFAULT_TOP_P, DEFAULT_TOP_K,


    DEFAULT_MIN_P, DEFAULT_TYPICAL_P, DEFAULT_PRESENCE_PENALTY, DEFAULT_FREQUENCY_PENALTY,


    DEFAULT_REPEAT_PENALTY, GPU_DEFAULT_MAX_TOKENS,


    DEFAULT_MIROSTAT_MODE, DEFAULT_MIROSTAT_ETA, DEFAULT_MIROSTAT_TAU,
...
def load_model(model_path, n_ctx=None, n_gpu_layers=None, chat_handler=None, force_cpu=False):
    ...
    if n_ctx is None:
        n_ctx = FAMILY_CONTEXT.get(model_family, FAMILY_CONTEXT.get("default", 32768))
    print(f"[llama] 上下文: n_ctx={n_ctx} (family={model_family})")
...
def infer(prompt=None, messages=None, images=None, system=None, stream=False, **params):
    ...
    gen_params = {
        ...
        "min_p": params.get("min_p") if params.get("min_p") is not None else DEFAULT_MIN_P,


        "typical_p": params.get("typical_p") if params.get("typical_p") is not None else DEFAULT_TYPICAL_P,


        "present_penalty": params.get("presence_penalty", DEFAULT_PRESENCE_PENALTY),


        "frequency_penalty": params.get("frequency_penalty", DEFAULT_FREQUENCY_PENALTY),


        "repeat_penalty": params.get("repeat_penalty") or DEFAULT_REPEAT_PENALTY,


        "mirostat_mode": params.get("mirostat_mode") if params.get("mirostat_mode") is not None else DEFAULT_MIROSTAT_MODE,


        "mirostat_eta": params.get("mirostat_eta") if params.get("mirostat_eta") is not None else DEFAULT_MIROSTAT_ETA,


        "mirostat_tau": params.get("mirostat_tau") if params.get("mirostat_tau") is not None else DEFAULT_MIROSTAT_TAU,
    ...
    _fam_defaults = FAMILY_SAMPLING_DEFAULTS.get(model_family) or FAMILY_SAMPLING_DEFAULTS.get("default", {})


    for _k, _v in _fam_defaults.items():


        if _k == "max_tokens":


            continue  # 由前端 slider 控制


        if _k == "presence_penalty":


            gen_params["present_penalty"] = _v  # llama-cpp-python 参数名
