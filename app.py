...
if True:  # llama-cpp routes (always registered, runtime check per-handler)
    ...
    def api_llama_infer():
        ...
        # 默认关闭 —— 模板、增强等全量 system prompt 不应被截断

        min_sys = data.get("min_prompt", False) or data.get("min_sys_prompt", False)
        ...
        if tools:
            ...
            # ── 技能目录注入（所有模型，工具开关打开时） ──
            # 不列出具体技能名（避免 LLM 误当成工具直接调用），只引导调用 skills_list
            try:
                from services.skill_loader import list_skills_meta
                skill_list = list_skills_meta()
                if skill_list:
                    count = len(skill_list)
                    skill_prompt = (
                        f"\n## 技能系统\n"
                        f"你有 {count} 个专业技能可用，涵盖视频生成、动画制作、图片生成等领域。\n"
                        f"当用户提出视频/动画/图片创作需求时，**第一步必须先调用 skills_list 工具**查看可用技能。\n"
                        f"然后按需调用 skill_view 获取技能指令、skill_reference 获取参考文献。\n"
                        f"不要直接调用技能名——它们不是工具函数。"
                    )
                    if "messages" in data and data["messages"]:
                        msgs = data["messages"]
